...
function statusLabel(status: DerivedStatus) {
  ...
  if (status === "pronto") return "â— PRONTO";
  if (status === "alerta") return "âš  ALERTA";
  return "OCUPADA";
}
...
export default function MesasView({ companyId }: { companyId: string }) {
  ...
  const criarMesa = async (numero: string, capacidade: number) => {
    if (!numero.trim()) return toast.error("Informe o nÃºmero da mesa");
  ...
  const adicionarItens = async (mesa: Mesa, modalItems: ModalItem[]) => {
    ...
    await supabase.from("mesas").update({ status: "ocupada" }).eq("id", mesa.id);
    toast.success("Itens adicionados Ã  comanda");
  ...
  const imprimirComanda = (info: MesaComputed) => {
    const html = `
      ...
      </style></head><body><h1>${APP_NAME} â€” Mesa ${info.mesa.numero}</h1><p>${info.pessoas} pessoa(s) Â· ${fmtTempo(info.tempo)}</p><table>${info.items
  ...
  return (
    <div className="mesas-modelo-root">
      ...
      <div className="mesas-toolbar">
        <div className="chips">
          {([
            ["todos", "Todas"],
            ["ocupada", "ðŸ”´ Ocupadas"],
            ["pronto", "ðŸŸ¢ Com pedido pronto"],
            ["alerta", "âš ï¸ Aguardando muito"],
            ["livre", "âšª Livres"],
        ...
        <div className="toolbar-right">
          ...
          <span className="toolbar-count">
            {mesas.length} mesas Â· {stats.ocupadas} ocupadas
          </span>
        </div>
      </div>
            ...
            <div className="mesa-list">
              {filteredMesas.map((info) => (
                <button key={info.mesa.id} className="mesa-list-row" onClick={() => (info.status === "livre" ? setShowComandaMesa(info.mesa) : setSelectedMesaId(info.mesa.id))}>
                  ...
                  <span>â± {fmtTempo(info.tempo)}</span>
                </button>
              ))}
            </div>
          )}
      ...
      {showComandaMesa && (
        <ComandaItemsModal
          title={`Nova Comanda â€” Mesa ${showComandaMesa.numero}`}
      ...
      {showAddItemMesa && (
        <ComandaItemsModal
          title={`Adicionar itens â€” Mesa ${showAddItemMesa.numero}`}
          mesa={showAddItemMesa}
          produtos={produtos}
          pizzaSizes={pizzaSizes}
          pizzaBordas={pizzaBordas}
          pizzaBordaPrecos={pizzaBordaPrecos}
          confirmLabel="âœ“ Adicionar itens"
      ...
      <div className="mesa-body">
        ...
        {livre ? (
          <div className="mesa-livre-body">
            <div className="mesa-livre-icon">ðŸª‘</div>
            <div className="mesa-livre-text">Mesa disponÃ­vel</div>
          ...
          <>
            <div className="mesa-pessoas">
              ...
              {identificacao && <span className="mesa-ident">Â· {identificacao}</span>}
            </div>

            <div className="mesa-resumo">
              {items.slice(0, 3).map((item) => (
                <div className="mesa-item-row" key={item.id}>
                  <span className="mesa-item-qty">{item.quantidade}Ã—</span>
            ...
            <div className="mesa-footer">
              ...
              <div className={`mesa-tempo ${tempo > 60 ? "danger" : tempo > 30 ? "warning" : ""}`}>â± {fmtTempo(tempo)}</div>
            </div>

            <div className="mesa-actions">
              <button className="mact-btn primary" onClick={(event) => { event.stopPropagation(); onAdd(); }}>+ Item</button>
              <button className="mact-btn success" onClick={(event) => { event.stopPropagation(); onCloseAccount(); }}>ðŸ’³ Fechar</button>
              <button className="mact-btn" onClick={(event) => { event.stopPropagation(); onSelect(); }} title="Ver comanda">ðŸ“‹</button>
            </div>
          </>
        )}
      </div>
        ...
        <>
          <div className="dp-header">
            ...
            <div className="dp-title-actions">
              <span className={`mesa-status-badge ${statusClass(info.status)}`}>{statusLabel(info.status)}</span>
              <button className="dp-close" onClick={onClose}>Ã—</button>
            </div>
          </div>

          <div className="dp-body">
            <div>
              <div className="dp-section-label">InformaÃ§Ãµes</div>
              <div className="info-box">
                <div><div className="info-muted">Mesa</div><strong>{info.mesa.numero}</strong></div>
                <div><div className="info-muted">Pessoas</div><strong>ðŸ‘¥ {info.pessoas}</strong></div>
                <div><div className="info-muted">Aberta hÃ¡</div><strong>â± {fmtTempo(info.tempo)}</strong></div>
                {info.identificacao && <div><div className="info-muted">IdentificaÃ§Ã£o</div><strong className="amber-text">{info.identificacao}</strong></div>}
              </div>
            </div>
              ...
              <div className="comanda-list">
                {info.items.map((item) => (
                  <div className="comanda-item" key={item.id}>
                    <div className="ci-qty">{item.quantidade}Ã—</div>
                    <div className="ci-info">
                      <div className="ci-name">{item.produto_nome}</div>
                      {item.observacoes && <div className="ci-obs">âš  {item.observacoes}</div>}
                    </div>
          ...
          <div className="dp-actions">
            <button className="dp-btn primary" onClick={() => onAddItem(info.mesa)}>+ Adicionar item Ã  comanda</button>
            <div className="dp-btn-row">
              <button className="dp-btn success" onClick={() => onFechar(info.mesa)}>ðŸ’³ Fechar conta</button>
              <button className="dp-btn" onClick={() => onPrint(info)}>ðŸ–¨ï¸ Imprimir</button>
            </div>
            <div className="dp-btn-row">
              <button className="dp-btn" onClick={() => onTransfer(info.mesa)}>â†”ï¸ Transferir</button>
              <button className="dp-btn danger" onClick={() => onCancel(info.mesa)}>âœ• Cancelar mesa</button>
            </div>
          </div>
        </>
      )}
...
function TotalBox({ total, service, finalTotal, totalTone }: { total: number; service: number; finalTotal: number; totalTone: "fire" | "green" }) {
  return (
    <div className="comanda-total-box">
      ...
      <div className="ct-row"><span>ServiÃ§o (10%)</span><span>{brl(service)}</span></div>
...
function NovaMesaModal({ onClose, onConfirm }: { onClose: () => void; onConfirm: (numero: string, capacidade: number) => void }) {
  ...
  return (
    <ModalShell
      title="Nova Mesa"
      onClose={onClose}
      footer={<><button className="mf-btn" onClick={onClose}>Cancelar</button><button className="mf-btn primary" onClick={() => onConfirm(numero, capacidade)}>âœ“ Criar Mesa</button></>}
    >
...
function ProductConfigDialog({
  ...
  const handleAdd = () => {
    ...
    if (isPizza && selectedSize) {
      ...
      const fraction = total === 2 ? "Â½" : total === 3 ? "â…“" : total === 4 ? "Â¼" : "";
      const baseName = total === 1
        ? `${produto.nome} (${selectedSize.label})`
        : `${allNames.map((n) => `${fraction} ${n}`).join(" / ")} (${selectedSize.label})`;
      composedName = selectedBorda ? `${baseName} â€¢ Borda ${selectedBorda.nome}` : baseName;
    ...
    >
      <div style={{ background: "var(--card, #1a1a1a)", borderRadius: 12, maxWidth: 560, width: "100%", maxHeight: "90vh", display: "flex", flexDirection: "column", border: "1px solid var(--border, #333)" }}>
        <div style={{ padding: 16, borderBottom: "1px solid var(--border, #333)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={onClose} style={{ background: "transparent", border: 0, color: "inherit", fontSize: 20, cursor: "pointer" }}>âœ•</button>
        </div>
        <div style={{ padding: 16, overflowY: "auto", flex: 1 }}>
          ...
          {isPizza && (
            <>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>ðŸ“ Escolha o tamanho</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(110px,1fr))", gap: 6, marginBottom: 14 }}>
                {SIZE_OPTIONS.map((s) => (
                  <button key={s.id} onClick={() => { setSizeId(s.id); setExtraFlavors((prev) => prev.slice(0, s.maxFlavors - 1)); }}
                    ...
                    <div style={{ fontSize: 10, color: "var(--text3, #999)" }}>{s.maxFlavors} sab Â· {s.slices} fat</div>
                  </button>
                ))}
              </div>
              ...
              {selectedSize && selectedSize.maxFlavors > 1 && (
                <>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>
                    ðŸ• Sabores adicionais <span style={{ fontSize: 11, color: "var(--text3, #999)" }}>(atÃ© {maxExtras} Â· {selectedExtraIds.length} selecionado{selectedExtraIds.length !== 1 ? "s" : ""})</span>
                  </div>
                  ...
                  <div style={{ maxHeight: 180, overflowY: "auto", marginBottom: 14, border: "1px solid var(--border, #333)", borderRadius: 8 }}>
                    {visibleFlavors.map((f) => {
                      ...
                      return (
                        <button key={f.id} onClick={() => toggleFlavor(f.id)}
                          ...
                          <span style={{ width: 18, height: 18, border: "1.5px solid var(--fire, #ff6b1a)", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>{sel ? "âœ“" : ""}</span>
                    ...
                    {!visibleFlavors.length && <div style={{ padding: 12, textAlign: "center", fontSize: 12, color: "var(--text3, #999)" }}>Nenhum sabor disponÃ­vel.</div>}
                  </div>
                </>
              )}
              ...
              {selectedSize && pizzaBordas.length > 0 && (
                <>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>ðŸ¥¯ Borda recheada <span style={{ fontSize: 11, color: "var(--text3, #999)" }}>(opcional)</span></div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: 6, marginBottom: 14 }}>
                    <button onClick={() => setBordaId("")} style={{ padding: 8, borderRadius: 8, border: !bordaId ? "2px solid var(--fire, #ff6b1a)" : "1.5px solid var(--border, #333)", background: !bordaId ? "rgba(255,69,0,.1)" : "transparent", cursor: "pointer", color: "inherit" }}>
                      ...
                      <div style={{ fontSize: 10, color: "var(--fire2, #ff8c1a)" }}>GrÃ¡tis</div>
                    </button>
                    {pizzaBordas.map((b) => {
          ...
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6 }}>ðŸ“ ObservaÃ§Ãµes <span style={{ fontSize: 11, color: "var(--text3, #999)" }}>(opcional)</span></div>
        ...
        <div style={{ padding: 12, borderTop: "1px solid var(--border, #333)", display: "flex", gap: 8, alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, border: "1px solid var(--border, #333)", borderRadius: 8, padding: "4px 8px" }}>
            <button onClick={() => setQty((q) => Math.max(1, q - 1))} style={{ background: "transparent", border: 0, color: "inherit", fontSize: 16, cursor: "pointer" }}>âˆ’</button>
          ...
          <button className="mf-btn primary" style={{ flex: 1 }} onClick={handleAdd} disabled={isPizza && !selectedSize}>
            ðŸ›’ Adicionar {brl(finalPrice * qty)}
          </button>
        </div>
      </div>
    </div>
...
function FecharContaModal({ info, onClose, onConfirm }: { info?: MesaComputed; onClose: () => void; onConfirm: (forma: string) => void }) {
  ...
  if (!info) return null;
  const methods = [
    ["pix", "ðŸ’ ", "Pix"], ["credito", "ðŸ’³", "CrÃ©dito"], ["debito", "ðŸ’³", "DÃ©bito"],
    ["dinheiro", "ðŸ’µ", "Dinheiro"], ["misto", "ðŸ”€", "Misto"], ["dividir", "ðŸ‘¥", "Dividir"],
  ];
  return (
    <ModalShell
      title={`Fechar Conta â€” Mesa ${info.mesa.numero}`}
      onClose={onClose}
      footer={<><button className="mf-btn" onClick={onClose}>Cancelar</button><button className="mf-btn primary" onClick={() => onConfirm(forma)}>âœ“ Confirmar Pagamento</button></>}
      wide
    >
...
function TransferirMesaModal({ origem, mesasLivres, onClose, onConfirm }: { origem: Mesa; mesasLivres: Mesa[]; onClose: () => void; onConfirm: (destinoId: string) => void }) {
  ...
  return (
    <ModalShell
      title={`Transferir â€” Mesa ${origem.numero}`}
      onClose={onClose}
      footer={<><button className="mf-btn" onClick={onClose}>Cancelar</button><button className="mf-btn primary" onClick={() => onConfirm(destino)}>âœ“ Transferir</button></>}
    >
      <FormField label="Mesa de destino">
        ...
        {!mesasLivres.length && <div className="empty-comanda">NÃ£o hÃ¡ mesas livres para transferir.</div>}
      </FormField>
    </ModalShell>
  );
}
